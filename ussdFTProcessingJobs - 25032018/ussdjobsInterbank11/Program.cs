...
namespace ussdjobsInterbank11
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                int cnt;
                string query = "Select * from Win32_Process Where Name = 'ussdjobsInterbank11.exe'";
        ...
        {
            string rangeval = ""; string amtrange = "";
            rangeval = ConfigurationManager.AppSettings["rangeval"].ToString();
            amtrange = ConfigurationManager.AppSettings["amtrange"].ToString();
            ...
            string sql = "select * from tbl_USSD_transfers where statusflag = 0 " +
                ...
                " and bankcode in " + rangeval + " and " + amtrange + " " +
                " and trans_type = 2 order by dateadded asc";
            ...
            else
            {
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    if (Ledcodeval == "1200" || Ledcodeval == "1201" || Ledcodeval == "1202" || Ledcodeval == "1203")
                    {
                        ...
                        sms = "Corporate customer is not allowed to use this Service";
                        g.insertIntoInfobip(sms, mobile);
                        continue;//by continue means pick the next record
                    }
