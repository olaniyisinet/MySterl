...
namespace TakeUssdChargesMerchantCode
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                int cnt;
                string query = "Select * from Win32_Process Where Name = 'TakeUssdChargesMerchantCode.exe'";
        ...
        static DataSet getTransactions()
        {
            ...
            string sql = "select refid,sessionid,debitAccount,debitamt,transtypeid, statusflag from tbl_USSD_trnx_chrgTaken where statusflag in (0,3)  and (statusCode is null or statusCode = 'x51') and transtypeid = 4";
        ...
        static void processDS(DataSet ds)
        {
            ...
            string toAccountVat = "";  int Last4 = 0;

            decimal Merchant_vat = 0; decimal Merchant_amt = 0;
            ...
            Merchant_amt = decimal.Parse(ConfigurationManager.AppSettings["Merchant_amt"].ToString());
            Merchant_vat = decimal.Parse(ConfigurationManager.AppSettings["Merchant_vat"].ToString());
            ...
            else
            {
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    if (transtypeid == 4) //Merchant payment 
                    {
                        amt = Merchant_amt;
                        ...
                        rqt.Append("<PaymentReference>" + "Charge taken for ussd pay by merchant code with sid " + sid + "</PaymentReference>");
                        rqt.Append("</IBSRequest>");
                        string str = "";
                        str = rqt.ToString();
