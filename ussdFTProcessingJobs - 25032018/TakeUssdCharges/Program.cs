...
namespace TakeUssdCharges
{
    class Program
    {
        ...
        static DataSet getTransactions()
        {
            ...
            string sql = "select refid,sessionid,debitAccount,debitamt,transtypeid, statusflag from tbl_USSD_trnx_chrgTaken where statusflag in (0,3)  and (statusCode is null or statusCode = 'x51') and transtypeid = 1";
        ...
        static void processDS(DataSet ds)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    else if (transtypeid == 4) //Merchant payment 
                    {
                        amt = Merchant_amt;
                        //mark the record for treating
                        UpdateRecord(88, refid);
