...
/// <summary>
/// Summary description for PinChange
/// </summary>
public class PinChange : BaseEngine
{
    ...
    {
        int cnt = 0; string pan = ""; string op = "Pin Change";
        Gadget g = new Gadget();
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            ...
            for (int i = 0; i < payload.Length; i++)
            {
                ...
                string paninfo = panobj + "|" + expdate + "|" + seqnum;
    ...
    public string SaveCurrentPin(UReq req)
    {
        string resp = "0";
        string prms = getParams(req);
        NameValueCollection prm = splitParam(prms);
        string panonly = prm["PAN"];
        string exp = prm["ExpDate"];
        ...
        try
        {
            ...
            {
                CardPinService.CardsSoapClient ws = new CardPinService.CardsSoapClient();
                string checkpin = ws.ValidateCardPIN(panonly, seqnum, exp, req.Msg);
                if (checkpin == "00")
                {
                    //carry on 
                }
    ...
    public string DisplaySummary(UReq req)
    {
        ...
        resp = "Are you sure you want to set a NEW pin on %0A" + maskedpan + "%0A1. Yes%0A2. No";
    ...
    public string DoSubmit(UReq req)
    {
        ...
        try
        {
            ...
            if (summary == 2)
            {
                resp = "Operation cancelled. Please dial *822*18# or select from *822# menu to try again.";
                return resp;
            }
            else
            {
                ...
                string[] response = ws.GetSelectPINAndUpdatePINOffset(pan, seqnum, exDate, newPin);
                if (response[0].StartsWith("00"))
                {
                    resp = "Pin successfully changed";
                    status = "Success";
                }
                else
                {
                    resp = "Pin couldn't be changed. %0A An error occurred.";
                    status = "Failed";
                }
