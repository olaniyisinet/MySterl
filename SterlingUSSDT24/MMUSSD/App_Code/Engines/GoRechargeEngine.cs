...
public class GoRechargeEngine : BaseEngine
{
    ...
    public string SaveRequest(UReq req)
    {
        ...
        try
        {
            ...
            if (nuban.StartsWith("11"))
            {
                ...
                if (ConvertMobile234((bank1.phoneNumber)) != req.Msisdn)
                {
                    cnt = 105;
                    addParam("cnt", cnt.ToString(), req);
                    req.next_op = 122;
    ...
    public string SaveNUBAN(UReq req)
    {
        ...
        try
        {
            ...
            if (nuban.StartsWith("11"))//bank one customer
            {
                try
                {
                    ...
                    if (bank1.phoneNumber != ConvertMobile080(req.Msisdn))
                    {
                        cnt = 105;
                        addParam("cnt", cnt.ToString(), req);
                        req.next_op = 122;
            ...
            else if (nuban.StartsWith("05"))//Imal Customers
            {
                var isMobValid = iMALEngine.validateMobileAndNuban(nuban, req.Msisdn);
                if (!isMobValid)
                {
            ...
            else
            {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    mobile = ConvertMobile234(mobile);
    ...
    public string ValidateBVN(UReq req)
    {
        ...
        else if (nuban.StartsWith("11"))
        {
            ...
            {
                var dbBvn = dsOne.Tables[0].Rows[0]["Bvn"].ToString();
                if (dbBvn != bvn)
                {
                    cnt = 102;
                    addParam("cnt", cnt.ToString(), req);
    ...
    public string DoBuySelf(UReq req)
    {
        ...
        try
        {
            ...
            flag = int.Parse(prm["A"]);
            if (flag == 3)//came from short string
            {
                //frmNuban = "";
    ...
    public string CollectAmtOthers(UReq req)
    {
        ...
        if (cnt == 0)
        {
            ...
            {
                removeParam("cnt", req);
                resp = "You have no active account for this service";
    ...
    public string SaveCoreRequest(UReq req)
    {
        //Testing redirect to dot core
        //*822*99*1# - Airtime || *822*99*2# - Data
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect();
        c.SetProcedure("spd_getRegisteredUserProfile");
        c.AddParam("@mobile", req.Msisdn.Trim());
        DataSet ds = c.Select("rec");
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                case "1":
                    addParam("Method", "1", req);
                    break;
                case "2":
                    addParam("Method", "2", req);
                    break;
                case "3":
                    addParam("Method", "3", req);
                    break;
                default:
                    //
                    break;
    ...
    public string RouteAirtimeToDotCore(UReq req)
    {
        //Redirect to dot core
        //from *822# menu 
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect();
        c.SetProcedure("spd_getRegisteredUserProfile");
        c.AddParam("@mobile", req.Msisdn.Trim());
    ...
    public string RouteDataToDotCore(UReq req)
    {
        //Redirect to dot core
        //from next page on menu  
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect();
        c.SetProcedure("spd_getRegisteredUserProfile");
        c.AddParam("@mobile", req.Msisdn.Trim());
        DataSet ds = c.Select("rec");
    ...
    public string DirectToDotCore(UReq req)
    {
        ...
        int method = RunCount(prm["Method"]);
        switch (method)
        {
            case 1://Redirect for Airtime(self)
                resp = "*822*82*1#";
                break;
            case 2://Redirect for Airtime(others)
                resp = "*822*82*2#";
                break;
            case 3://Redirect for DATA
                resp = "*822*82*4#";
                break;
            default:
                resp = "*822*82#";
