...
public class BaseEngine
{
    ...
    public int RunBankPage(string s)
    {
        int k = 0;
        try
        {
    ...
    public DataSet getImalAcctsByMobile(string mobileNo)
    {
        ...
        try
        {
            ...
            string sqlData = @"SELECT AMF.ADDITIONAL_REFERENCE as Nuban, (amf.cv_avail_bal + amf.BLOCKED_CV) as Balance, amf.gl_code as LedgerCode FROM IMAL.CIF INNER JOIN IMAL.AMF ON CIF.CIF_NO = AMF.CIF_SUB_NO WHERE amf.gl_code in (210801,210153,210804,210805,210201,210101) and CIF.cif_no = '" + custId + "'";
    ...
    public string GetPrimaryAccount(string mobileNum)
    {
        string acct = "";
        string sql = "select Nuban from tbl_Default_Accounts where Mobile=@mobile";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect();
        c.SetSQL(sql);
        c.AddParam("@mobile", mobileNum.Trim());
        DataSet ds = c.Select("rec");
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            DataRow dr = ds.Tables[0].Rows[0];
            acct = dr["Nuban"].ToString();
        }
        else
        {
            acct = "";
    ...
    public string GetMostUsedAccount(string mobileNo)
    {
        string acct = "";
        string sql = "select frmAccount,COUNT(frmAccount) as AcctCount from tbl_USSD_transfers where mobile = @mobile and frmAccount is not null group by frmAccount" +
                " union" +
                " select frmAccount,COUNT(frmAccount) as AcctCount from tbl_USSD_transfers_batch where mobile = @mobile and frmAccount is not null group by frmAccount" +
                " order by AcctCount desc";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect();
        c.SetSQL(sql);
        c.AddParam("@mobile", mobileNo.Trim());
        DataSet ds = c.Select("rec");
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            DataRow dr = ds.Tables[0].Rows[0];
            acct = dr["frmAccount"].ToString();
        }
        else
        {
            acct = "";
    ...
    public string GetBankShortName(int bankID)
    {
        string bankName = "";
        string sql = "select * from tbl_participatingBanks where TransRate=@id";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect("mssqlconn_nip");
        c.SetSQL(sql);
        c.AddParam("@id", bankID);
        DataSet ds = c.Select("rec");
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            DataRow dr = ds.Tables[0].Rows[0];
            bankName = dr["bankshort"].ToString();
        }
        else
        {
            bankName = "";
    ...
    public bool CheckIfBeneficiaryIsSaved(int toBankId, string toNuban, string mobile)
    {
        bool resp = false;
        //string sql = "select * from tbl_SavedBeneficiary where BankCode=@id and Nuban=@nuban";
        string sql = "select * from tbl_SavedBeneficiaryWithMobile where BankCode=@id and Nuban=@nuban and SourceMobile=@mob";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect("mssqlconn");//tbl_SavedBeneficiaryWithMobile
        c.SetSQL(sql);
        c.AddParam("@id", toBankId);
    ...
    public bool checkForPin2(string nuban, string mobile, UReq req)
    {
        ...
        else
        {
            ...
            if (response.StartsWith("00"))
            {
                //check acct with wise bank first 
                //use cnt = 112
    ...
    public string getUSSDPin1(string mobile, string nuban)
    {
        string pin = "";
        string sql = "SELECT custauthid FROM Go_Registered_Account where Mobile = @mob and NUBAN = @acct";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect("mssqlconn");
        c.SetSQL(sql);
        c.AddParam("@mob", mobile);
        ...
        {
            DataRow dr = ds.Tables[0].Rows[0];
            pin = dr["custauthid"].ToString();

            Encrypto EnDeP = new Encrypto();
            pin = EnDeP.Decrypt(pin);
        }
        else
        {
            pin = "";
    ...
    public string getUSSDPin2(string mobile, string nuban)
    {
        string pin = "";
        string sql = "SELECT custauthid2 FROM Go_Registered_Account where Mobile = @mob and NUBAN = @acct";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect("mssqlconn");
        c.SetSQL(sql);
        c.AddParam("@mob", mobile);
        c.AddParam("@acct", nuban);
    ...
    public string removeSpareParams(string originalParam)
    {
        string resp = "";
        try
        {
            char[] delm = { ',' };
            string msg = originalParam.Trim();
    ...
    public Tuple<string, string> GetLastOpenSession(UReq req)
    {
        string prevSession = "";
        string prevParams = "";
        Gadget g = new Gadget();
        ...
        if (req.Msg != "*822#")
        {
            return Tuple.Create(prevSession, prevParams);
        }

        string sql = "select top 1 * from tbl_USSD_reqstate where msisdn=@msi and op_id not in (3,990,99) and sub_op_id = 0 and lastUpdated > DATEADD(MINUTE,-10,GETDATE()) order by refID desc";//and params not like '%PrevSess%'
        Sterling.MSSQL.Connect cn = new Sterling.MSSQL.Connect();
        //cn.Persist = true;
        cn.SetSQL(sql);
        cn.AddParam("@msi", req.Msisdn);
        ...
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            prevSession = ds.Tables[0].Rows[0]["sessionid"].ToString();
            prevParams = ds.Tables[0].Rows[0]["params"].ToString();
        }
        else
        {
            //previous session was completed or was done longer than an hour ago
        }

        return Tuple.Create(prevSession, prevParams);
    }
    public void InsertPrevParam(UReq req, string prevParams)
    {
        string sql = "update tbl_USSD_reqstate set params=@params, lastUpdated = getdate()" +
                " where sessionid=@ssi and msisdn = @msi";
        Sterling.MSSQL.Connect cn = new Sterling.MSSQL.Connect();
        cn.SetSQL(sql);
        cn.AddParam("@ssi", req.SessionID);
        cn.AddParam("@msi", req.Msisdn);
        cn.AddParam("@params", prevParams);
        cn.Update();

    }

    public Tuple<int, int> GetLastOperation(string oldSessionID, string mobile)
    {
        int op_id = 0;
        int sub_op = 1;
        string sql = "select * from tbl_USSD_reqstate " +
            " where sessionid = @ssi and msisdn = @msi order by refID desc";
        Sterling.MSSQL.Connect cn = new Sterling.MSSQL.Connect();
        cn.SetSQL(sql);
        cn.AddParam("@ssi", oldSessionID);
        cn.AddParam("@msi", mobile);
        DataSet ds = cn.Select();
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            op_id = RunCount(ds.Tables[0].Rows[0]["op_id"].ToString());
            sub_op = RunCount(ds.Tables[0].Rows[0]["sub_op_id"].ToString());

            Tuple<int, int> displayId = GetDisplayOpSubOp(op_id, sub_op);
            op_id = displayId.Item1;
            sub_op = displayId.Item2;
        ...
        return Tuple.Create(op_id, sub_op);
    }

    public Tuple<int, int> GetDisplayOpSubOp(int op_id, int sub_op_id)
    {

        int next_Op = 0;
        string menu = ""; string opMethod = "";
        bool result = false;
        string[] methods = { "SaveRequest", "CheckNoofAccts" };

        do
        {
            string sql = "select * from tbl_USSD_states where op_id=@op and op_resp=@sub_op";
            Sterling.MSSQL.Connect cn = new Sterling.MSSQL.Connect();
            cn.SetSQL(sql);
            cn.AddParam("@op", op_id);
            cn.AddParam("@sub_op", sub_op_id);
            DataSet ds = cn.Select();
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                op_id = RunCount(ds.Tables[0].Rows[0]["op_id"].ToString());
                sub_op_id = RunCount(ds.Tables[0].Rows[0]["op_resp"].ToString());
                next_Op = RunCount(ds.Tables[0].Rows[0]["next_op_id"].ToString());
                menu = ds.Tables[0].Rows[0]["op_menu"].ToString();
                opMethod = ds.Tables[0].Rows[0]["op_method"].ToString();
                if (string.IsNullOrEmpty(menu) && !methods.Contains(opMethod, StringComparer.OrdinalIgnoreCase))
                {
                    DataSet ds2 = GetPrevOpFromOpId(op_id);
                    op_id = RunCount(ds2.Tables[0].Rows[0]["op_id"].ToString());
                    sub_op_id = RunCount(ds2.Tables[0].Rows[0]["op_resp"].ToString());
                    continue;
                }
                else
                {
        ...
        return Tuple.Create(op_id, sub_op_id);

    }

    public DataSet GetPrevOpFromOpId(int opid)
    {
        string sql = "select * from tbl_USSD_states where next_op_id = @op order by op_resp";
        Sterling.MSSQL.Connect cn = new Sterling.MSSQL.Connect();
        cn.SetSQL(sql);
        cn.AddParam("@op", opid);
        DataSet ds = cn.Select();
    ...
    {
        string resp = "";
        resp = Convert.ToBase64String(Encoding.UTF8.GetBytes(value));
        return resp;
    }
    public int GetStepFromShortString(string reqmesg)
    {
        int nextOp = 0;

        char[] delm = { '*', '#' };
        string[] s = reqmesg.Split(delm, StringSplitOptions.RemoveEmptyEntries);
    ...
    public int GetNextOpFromWiseMainMenu(string menuNo)
    {
        int nextOp = 0;
        ...
        switch (menuNo)
        {
            ...
            case "3"://Open Account
                nextOp = 2;
                break;
            case "4"://Local transfer
                nextOp = 2540;
                break;
            case "5"://Inter bank transfer
                nextOp = 2560;
                break;
            case "6"://Airtime Self
                nextOp = 2521;
                break;
            case "7"://Airtime Others
                nextOp = 2530;
                break;
            case "8"://Next page
                nextOp = 2503;
    ...
    public int GetNextOpFromWiseMainMenu2(string menuNo)
    {
        int nextOp = 0;
    ...
    public long SaveRequestToWisebank(string sessionId, string methodName, string request)
    {
        long refid = 0;
        string sql = "insert into tbl_Wisebank_Logs (SessionID,MethodName,Request) values (@ssid,@method,@req)";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect();
        c.SetSQL(sql);
        c.AddParam("@ssid", sessionId);
        c.AddParam("@method", methodName);
        c.AddParam("@req", request);
        try
        {
            refid = Convert.ToInt64(c.Insert());
    ...
    public void SaveResponseFromWisebank(long refid, string response, bool respResult)
    {
        string sql = "Update tbl_Wisebank_Logs  set ResponseResult=@respcode, Response= @resp, Dateprocessed=@date where Refid = @refid";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect();
        c.SetSQL(sql);
        c.AddParam("@resp", response);
        c.AddParam("@respcode", respResult.ToString());
        c.AddParam("@date", DateTime.Now);
        c.AddParam("@refid", refid);
        c.Update();
