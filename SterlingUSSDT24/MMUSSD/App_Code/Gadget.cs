...
public class Gadget
{
    ...
    public List<string> GetT24CustIdList(string mobile)
    {
        string custid = "";
    ...
    //**************************
    public string GetBeneByListID(int id, UReq req)
    {
        string nuban = ""; string bankCode = "";
        string sql = "select * from tbl_USSD_BeneficiaryList where mobile =@mob and TransRate = @id and sessionid =@sid";
        Sterling.MSSQL.Connect c = new Sterling.MSSQL.Connect("mssqlconn");
        c.SetSQL(sql);
        c.AddParam("@mob", req.Msisdn);
        c.AddParam("@id", id);
        ...
        if (ds != null && ds.Tables[0].Rows.Count > 0)
        {
            DataRow dr = ds.Tables[0].Rows[0];
            nuban = dr["Nuban"].ToString();
            bankCode = dr["BeneBankCode"].ToString();

        }
        return nuban + "*" + bankCode;
    }
    public List<BeneList> GetBeneListByPage(string page, string sid)
    {
        List<BeneList> lb = new List<BeneList>();
        int pg = 0;
        try
        {
            pg = Convert.ToInt32(page);
        ...
        string sql = " SELECT top " + pageSize + " * FROM tbl_USSD_BeneficiaryList " +
                     " where statusflag = 1 and sessionid =@sid " +
                     " and shortname not in " +
                     " (select top " + offset.ToString("0") +
                     " shortname  from tbl_USSD_BeneficiaryList " +
                     " where statusflag = 1 and sessionid =@sid order by TransRate asc) " +
                     " order by TransRate asc";
        Sterling.MSSQL.Connect cn = new Sterling.MSSQL.Connect();
        cn.SetSQL(sql);
