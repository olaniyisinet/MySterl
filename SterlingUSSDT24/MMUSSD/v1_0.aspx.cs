...
public partial class v1_0 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            case 1: //new request
                    //testing previous status GetPreviousState
                BaseEngine bas = new BaseEngine();
                Tuple<string,string> lastOpenSession = bas.GetLastOpenSession(req);//item1 = Session, item 2= params
                if (!string.IsNullOrEmpty(lastOpenSession.Item1))
                {
                    //set steps for previous session.
                    req.op = 99;
                    req.sub_op = 0;
                    usv.saveState(req);
                    bas.InsertPrevParam(req, lastOpenSession.Item2);
                    bas.addParam("PrevSess", lastOpenSession.Item1, req);
