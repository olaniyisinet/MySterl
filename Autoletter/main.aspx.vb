...
Partial Class main
    ...
    Protected Sub Button1_Click(sender As Object, e As System.EventArgs) Handles Button1.Click
        ...
        Try
            ...
            If Not IsNumeric(TextBox3.Text) Then
                ...
                Label1.Text = "Kindly ensure the Customer Number is numeric."
            ...
            ElseIf RadioButton5.Checked Then
                selected = RadioButton5.Text
            ElseIf RadioButton6.Checked Then
                selected = RadioButton6.Text


            End If


            auto.AuditAction(Session("name").ToString(), "Selected " + selected)
            ...
            Using conn As New Data.SqlClient.SqlConnection(chk2.sqlconn())
                ...
                If String.IsNullOrEmpty(getCustnum) Then
                    auto.AuditAction(Session("name").ToString(), "is generating " + selected + " document for " + TextBox2.Text.Trim + " for date: " + dt)

                    'Dim cm As New Data.SqlClient.SqlCommand("select DISTINCT T24FDD2.* from T24FDD2,BranchInfo WHERE T24BRANCHID=CO_CODE AND MATURITY_DATE is null AND TERM is null AND (BANKSBRACODE=@BR OR T24BRANCHID=@BR)  AND START_DATE=@bid", conn)
                    ...
                    Dim cm As New Data.SqlClient.SqlCommand("select DISTINCT T24FDD2.* from T24FDD2,BranchInfo WHERE T24BRANCHID=CO_CODE AND MATURITY_DATE <> '' AND TERM <> ''  AND (BANKSBRACODE=@BR OR T24BRANCHID=@BR)  AND START_DATE=@bid", conn)
                    cm.Parameters.AddWithValue("@bid", dt)
                    cm.Parameters.AddWithValue("@BR", TextBox2.Text.Trim)
                    da.SelectCommand = cm

                Else
                    auto.AuditAction(Session("name").ToString(), "is generating " + selected + " document for customer " + TextBox3.Text.Trim + " of branch " + TextBox2.Text.Trim + " for date: " + dt)

                    Dim cm As New Data.SqlClient.SqlCommand("select DISTINCT T24FDD2.* from T24FDD2,BranchInfo WHERE T24BRANCHID=CO_CODE AND MATURITY_DATE <> '' AND TERM <> ''  AND (BANKSBRACODE=@BR OR T24BRANCHID=@BR)  AND START_DATE=@bid AND CUSTOMER=@cust", conn)
                    cm.Parameters.AddWithValue("@bid", dt)
                    cm.Parameters.AddWithValue("@BR", TextBox2.Text.Trim)
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    'System.IO.Directory.CreateDirectory(AppSettings("images").ToString() & bno)
                    ...
                    For Each dr As Data.DataRow In ds.Tables(0).Rows
                        ...
                        If RadioButton6.Checked Then
                            ...
                            ref = Convert.ToString(Val(dr("CUSTOMER").ToString) & Val(dr("CO_CODE").ToString) & Val(dr("AMOUNT").ToString.Replace(",", "").Replace(".", "")))
                        End If
                        ...
                        Dim phoneNo As String = dr("PHONE_1").ToString().Trim
                        ...
                        Dim valadd As Integer
                        Dim paddingadd = ""
                        Dim account As String = dr("LINKED_APPL_ID").ToString
                        If dr("TERM").ToString.Contains("D") Then
                            getday = dr("TERM").ToString.Replace("D", "")
                            valadd = 1
                            paddingadd = " DAYS"
                        ElseIf dr("TERM").ToString.Contains("M") Then
                            getday = dr("TERM").ToString.Replace("M", "")
                            valadd = 30
                            paddingadd = " MONTHS"
                            'Year tenor was not implemented, else the need to add this
                        ElseIf dr("TERM").ToString.Contains("Y") Then
                            getday = dr("TERM").ToString.Replace("Y", "")
                            valadd = 365
                            If getday > 1 Then
                                paddingadd = " YEARS"
                            Else
                        ...
                        Else
                            getday2 = getday
                        End If
                        Dim getdayval As Integer = CInt(getday2) * valadd
                        ...
                        Dim payOutAmt As Double
                        Dim payOutDt = CDate(dtt.AddMonths(61).ToString().Trim).ToString("dd-MMM-yyyy")
                        Dim prinContr As String
                        Dim prinContrComm As String
                        Dim tenorComm As String = ""
                        Dim mpoComm As String = "for 60 months effective"
                        Dim firstComm As String = "You are required to meet your monthly contribution to fully obtain the payout for the stated tenor."
                        Dim secComm As String = "However, premature termination of deposit will attract a penal rate of 50% on the interest earned."
                        Dim thirComm As String = "This is an auto generated letter and requires no signature."

                        prinContr = "Monthly Contribution"
                        prinContrComm = "for 60 months"
                        If RadioButton6.Checked Then

                            If getday = 10 Then
                                'payOutAmt = FormatNumber(valueamt * 150 / 100, 2)
                                payOutAmt = FormatNumber(dr("AMOUNT").ToString, 2) * 1.5
                                tenorComm = "5 years pay-in and 5 years pay-out period"

                            ElseIf getday = 15 Then
                                payOutAmt = FormatNumber(valueamt, 2)
                                tenorComm = "5 years pay-in and 10 years pay-out period"
                                mpoComm = "for 120 months effective"
                            ElseIf getday = 6 Then
                                payOutAmt = FormatNumber(valueamt * 125 / 100, 2)
                                payOutDt = CDate(dtt.AddMonths(37).ToString().Trim).ToString("dd-MMM-yyyy")
                                prinContrComm = "for 36 months"
                                tenorComm = "3 years pay-in and 3 years pay-out period"
                                mpoComm = "for 36 months effective"
                            ElseIf getday = 3654 Then
                                getday = 10
                                paddingadd = " YEARS"
                                payOutAmt = FormatNumber(valueamt * 200 / 6000, 2)
                                prinContr = "Principal"
                                prinContrComm = ""
                        ...
                        Dim anndtt As String



                        If dtt.Day = 1 Or dtt.Day = 21 Or dtt.Day = 31 Then
                            anndtt = dtt.Day.ToString().Trim + "st Of Every Month"
                        ElseIf dtt.Day = 2 Or dtt.Day = 22 Then
                            anndtt = dtt.Day.ToString().Trim + "nd Of Every Month"
                        ElseIf dtt.Day = 3 Or dtt.Day = 23 Then
                            anndtt = dtt.Day.ToString().Trim + "rd Of Every Month"

                        Else
                            anndtt = dtt.Day.ToString().Trim + "th Of Every Month"
                        End If

                        Dim myTI As TextInfo = CultureInfo.CurrentCulture.TextInfo
                        ...
                        test = CDate(dtt.ToString).ToString("dd-MMMM-yyyy")
                        strArray = Split(test, "-")

                        Dim myChar As Char() = {"0"c}

                        If strArray(0) = "01" Or strArray(0) = "21" Or strArray(0) = "31" Then
                            test1 = strArray(0).ToString().TrimStart(myChar) + "st Of " + strArray(1).ToString().Trim + ", " + strArray(2).ToString().Trim
                        ElseIf strArray(0) = "02" Or strArray(0) = "22" Then
                            test1 = strArray(0).ToString().TrimStart(myChar) + "nd Of " + strArray(1).ToString().Trim + ", " + strArray(2).ToString().Trim

                        ElseIf strArray(0) = "03" Or strArray(0) = "23" Then
                            test1 = strArray(0).ToString().TrimStart(myChar) + "rd Of " + strArray(1).ToString().Trim + ", " + strArray(2).ToString().Trim

                        Else
                            test1 = strArray(0).ToString().TrimStart(myChar) + "th Of " + strArray(1).ToString().Trim + ", " + strArray(2).ToString().Trim
                        ...
                        htmlstr2 = htmlstr.Replace("%date%", CDate(dtt.ToString).ToString("dd-MMM-yyyy")) _
                            ...
                            .Replace("%fname%", dr("SHORT_NAME").ToString) _
                             .Replace("%refno%", ref) _
                        .Replace("%address1%", myTI.ToTitleCase(dr("STREET").ToString.ToLower())) _
                        .Replace("%acctnum%", dr("LINKED_APPL_ID").ToString) _
                        .Replace("%address2%", myTI.ToTitleCase(dr("TOWN_COUNTRY").ToString.ToLower())) _
                                                .Replace("%amount%", FormatNumber(dr("AMOUNT").ToString, 2)).Replace("%currency%", cur) _
                                                .Replace("%interest%", dr("EFFECTIVE_RATE").ToString & " %") _
                                                .Replace("%days%", getday.ToString & paddingadd) _
                                                .Replace("%effectiveDate%", CDate(dtt.ToString).ToString("dd-MMMM-yyyy")) _
                                                .Replace("%MaturityDate%", CDate(matdtt.ToString).ToString("dd-MMMM-yyyy")) _
                                                .Replace("%AnnDate%", anndtt.ToString) _
                                                .Replace("%fDate%", test1.ToString) _
                                                .Replace("%PayOutAmt%", payOutAmt.ToString) _
                                                .Replace("%PayOutDt%", payOutDt.ToString) _
                                                .Replace("%prinContr%", prinContr.ToString) _
                                                .Replace("%prinContrComm%", prinContrComm.ToString) _
                                                .Replace("%tenorComm%", tenorComm.ToString) _
                                                .Replace("%mpoComm%", mpoComm.ToString) _
                                                .Replace("%firstComm%", firstComm.ToString) _
                                                .Replace("%secComm%", secComm.ToString) _
                                                .Replace("%thirComm%", thirComm.ToString) _
                                                .Replace("%phoneNo%", phoneNo.ToString) _
                        ...
                        If CheckBox1.Checked And Not RadioButton6.Checked Then
                            ...
                            If Not String.IsNullOrEmpty(dr("EMAIL_1").ToString()) Then
                                Try
                                    ...
                                    auto.sendmail(dr("EMAIL_1").ToString, "ebusiness@sterlingbankng.com", "TERM DEPOSIT LETTER FROM STERLING BANK", msgbody, oname)
                        ...
                        If CheckBox1.Checked And RadioButton6.Checked Then

                            If Not String.IsNullOrEmpty(dr("EMAIL_1").ToString()) Then
                                Try
                                    msgbody = "Dear " & dr("SHORT_NAME").ToString & "<br/><br/>" & "Find attached your Doubble By Sterling Confirmation letter.<br/>Please note your reference number for future verification.<br/><br/><b>Letter Reference No:</b> " & ref & "<br/><br/>Sterling Bank Plc"
                        ...
                        If CheckBox2.Checked And Not RadioButton6.Checked Then
                        ...
                        If CheckBox2.Checked And RadioButton6.Checked Then
                            If Not String.IsNullOrEmpty(dr("PHONE_1").ToString()) Then
                                Try
                                    msgbody = "Dear Customer, Your Doubble By Sterling Deposit letter is now ready.%0APlease call customer care on 01-4484481-5 for more info.%0ASterling Bank Plc"
                                    auto.sendSMS(dr("PHONE_1").ToString, msgbody)
                                Catch ex2 As Exception
                                    System.IO.File.AppendAllText(AppSettings("errorlog").ToString() & Year(Now) & Month(Now) & Day(Now) & ".txt", Now & ex2.Message & vbCrLf)
                    ...
                    auto.AuditAction(Session("name").ToString(), "generated " + selected + " document for customer " + TextBox3.Text.Trim + " of branch " + TextBox2.Text.Trim + " for date: " + dt)
    ...
    Sub createPDF(htmlstr As String, pdfname As String)
        ...
        Dim doc As New iTextSharp.text.Document(PageSize.A4, 7.0F, 7.0F, 7.0F, 0F)
        Dim fsNew As New StringReader(line)
        'Dim document As New Document(PageSize.A4, 80, 50, 30, 65)
        Dim document As New iTextSharp.text.Document(PageSize.A4, 7.0F, 7.0F, 7.0F, 0F)
