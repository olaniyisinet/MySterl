    ...
    Protected Sub Button1_Click(sender As Object, e As System.EventArgs) Handles Button1.Click
        Try
            Using conn As New Data.SqlClient.SqlConnection(chk2.sqlconn())
                ...
                Try
                    auto.AuditAction(Session("name").ToString(), "Spooled BA records for Branch " + TextBox2.Text.Trim() + " for date " + dt.ToString())
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    auto.AuditAction(Session("name").ToString(), "Spooled " + ds.Tables(0).Rows.Count.ToString() + " ba records for Branch " + TextBox2.Text.Trim() + " for date " + dt)



                Else
                    auto.AuditAction(Session("name"), "Spooled 0 ba records for Branch " + TextBox2.Text.Trim() + " for date " + dt)
        ...
        Catch ex As Exception
            auto.AuditAction(Session("name"), "Encountered error while spooling BA records for Branch " + TextBox2.Text.Trim() + " for date " + TextBox1.Text.Trim.Replace("-", ""))
        ...
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                For Each dr As Data.DataRow In ds.Tables(0).Rows
                    ...
                    'auto.AuditAction(Session("name"), "Started the generation of BA Document for Customer, " + dr("CUS_SHO_NAME").ToString + " with cusnum " + Val(dr("CUS_NUM").ToString + " of branch " + TextBox2.Text.Trim() + " for date " + TextBox1.Text.Trim.Replace("-", "")))
                    ...
                    If CheckBox1.Checked Then
                        If Not String.IsNullOrEmpty(dr("EMAIL").ToString()) Then
                            Try
                                auto.AuditAction(Session("name"), "Sent BA Document for Customer, " + dr("CUS_SHO_NAME").ToString + " with cusnum " + Val(dr("CUS_NUM").ToString + " of branch " + TextBox2.Text.Trim() + " for date " + TextBox1.Text.Trim.Replace("-", "")))
                    ...
                    'auto.AuditAction(Session("name"), "Downloaded BA Document for Customer, " + dr("CUS_SHO_NAME").ToString + " with cusnum " + Val(dr("CUS_NUM").ToString + " of branch " + TextBox2.Text.Trim() + " for date " + TextBox1.Text.Trim.Replace("-", "")))
